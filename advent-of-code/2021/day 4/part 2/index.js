...
function startGame(boards, draws) {
    let playBoards = [...boards];
    let lastWonBoardSum;
    for (const draw of draws) {
        if(playBoards.length === 0) {
            console.log('All boards have won, exiting game');
            return;
        }
        console.log(`:: Drawing [${draw}] for ${playBoards.length} boards`);
        drawNumber(draw, playBoards);
        const finishedBoards = [];
        for (const currentBoard of playBoards) {
            const boardIsAWinner = checkBoardForWin(currentBoard, draw);
            if(boardIsAWinner !== undefined) {
                lastWonBoardSum = boardIsAWinner;
                finishedBoards.push(currentBoard);
            }
        }
        if(finishedBoards.length > 0) {
            playBoards = playBoards.filter((entry) => !finishedBoards.includes(entry));
        }
    }
    console.log('LAST winner:', lastWonBoardSum);
}
...
function checkBoardForWin(board, lastDraw) {
    if (board.sum.rows.includes(5) || board.sum.columns.includes(5)) {
        ...
        console.log('-------------------------------');
        return sumOfUnmarkedFields * lastDraw;
    }
    return;
}
